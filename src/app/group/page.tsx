...
export default function HomePage() {
  ...
  // Verificar si se aplicaron los topics y mostrar el mensaje de éxito
  useEffect(() => {
    if (localStorage.getItem("topicsApplied") === "true") {
      setShowSuccessMessage(true);
      localStorage.removeItem("topicsApplied"); // Remover el estado para no mostrarlo cada vez
    }
  }, []);
  ...
  useEffect(() => {
    setTimeout(() => {
      const success = localStorage.getItem("topicsApplied");
      if (success) {
        setShowSuccess(true);
        localStorage.removeItem("topicsApplied");
  
        setTimeout(() => setShowSuccess(false), 2000); // Oculta el mensaje después de 2 segundos
      }
    }, 100); // Retraso para asegurar que la redirección haya terminado
  }, []);
